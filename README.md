...
You don't need to set `SEO_JS_BACKEND`, since it defaults to prerender.io.
...
- That pre-rendered HTML is generated by a service with a headless browser that runs your js, then caches the page.
- Said service is generally a third party (there are many: [prerender.io](https://prerender.io/), [Brombone](http://www.brombone.com/), [seo.js](http://getseojs.com/), [seo4ajax](http://www.seo4ajax.com/)). You can also run the service yourself, using [prerender](https://github.com/prerender/prerender), or re-inventing the wheel for fun.

Django-seo-js supports both the `?_escaped_fragment_` and user-agent approaches, so it's agnostic as to how you structure your app.
